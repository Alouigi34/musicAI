...
for i in range(89):    
    lmodels2.append(LiteModel.from_keras_model(loaded_models2[i]))
for i in range(89):    
    lmodels3.append(LiteModel.from_keras_model(loaded_models3[i]))
...
#example 3 is to check model's accuracy with given piece
...
if example == 3:
    ...
    real_notes = []    
    for i in range(89):    
        real_notes.append(output_data1[i][index_check][0][0])
    #De-Normalize each feature indipendently:
    for i in range(89):
            value = (real_notes[i])
            if example == 3:
                real_notes[i] = denormalize(value,
                                            characteristics_min[i],
                                            characteristics_max[i])
    real_notes=np.array(real_notes)    
    real_notes[np.isnan(real_notes)] = 0
    real_notes[real_notes<4] = 0
    real_notes=np.round(real_notes)     
    print("Actual:")        
    #print(real_notes)
    for i in range(0,90,10): # print 10 rows and 10 coloumns at a time
        print(real_notes[i:i+10])
    ...
    # real_notes2 = []    
    # for i in range(89):    
    #     real_notes2.append(output_data2[i][index_check][0][0])
    # #De-Normalize each feature indipendently:
    # for i in range(89):
    #         value = (real_notes2[i])
    #         if example == 3:
    #             real_notes2[i] = denormalize(value,
    #                                         characteristics_min2[i],
    #                                         characteristics_max2[i])
    # real_notes2=np.array(real_notes2)    
    # real_notes2[np.isnan(real_notes2)] = 0
    # real_notes2[real_notes2<4] = 0
    # real_notes2=np.round(real_notes2)     
    # print("Actual 2:")        
    # #print(real_notes2)    
    # for i in range(0,90,10): # print 10 rows and 10 coloumns at a time
    #     print(real_notes2[i:i+10])    
